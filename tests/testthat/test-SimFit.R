...
test_that("Simfit_diff_dispersions",{
  ...
  expect_equal(c(mymod.1$theta,mymod.2$theta), fit.diff.disp$genCoeff.function(FALSE)$gamma)
  mu <- matrix(rep(exp(c(coefficients(mymod.1),coefficients(mymod.2))),100),ncol=2,byrow=TRUE)
  names(mu) <- NULL
  expect_equal(mu, fit.diff.disp$genCoeff.function(FALSE)$mu)
})
...
test_that("summary.SingleSimFit",{
  ...
  expect_equal(fit$genCoeff.function(FALSE)$mu[1,], s$rate.estimate)
  expect_equal(fit$model$df.residual,s$df)
  expect_equal("dropout",s$datastatus)
  expect_equal(1/fit$genCoeff.function(FALSE)$gamma[1], s$dispersion)
