...
test_that("Simfit_diff_dispersions",{
  ...
  expect_equal(c(mymod.1$theta,mymod.2$theta), fit.diff.disp$genCoeff.function()$gamma)
  mu <- matrix(rep(exp(c(coefficients(mymod.1),coefficients(mymod.2))),100),ncol=2,byrow=TRUE)
  names(mu) <- NULL
  expect_equal(mu, fit.diff.disp$genCoeff.function()$mu)
})
...
test_that("sim_fit_equal_dispersion",{
  ...
  expect_equal(rep(mymod$theta,2),fit$genCoeff.function()$gamma)
  
  #checking mu
  mu <- mu <- matrix(rep(exp(mymod$coefficients[1])*c(1,exp(mymod$coefficients[2])),100),ncol=2,byrow=TRUE)
  names(mu) <- NULL
  expect_equal(mu,fit$genCoeff.function()$mu)
...
test_that("summary.SingleSimFit",{
  ...
  expect_equal(fit$genCoeff.function()$mu[1,], s$rate.estimate)
  expect_equal(fit$model$df.residual,s$df)
  expect_equal("dropout",s$datastatus)
  expect_equal(1/fit$genCoeff.function()$gamma[1], s$dispersion)
