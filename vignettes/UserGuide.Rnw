...
The \texttt{LinearRateChangeDrop} function allows a piecewise exponential drop out function where after $j$ events subjects have drop out rate $R_j$ where $R_j = C_j\exp(X_j)$ where $X_j \sim \mathcal{N}(0,\sigma^2)$ and $C_j = C+j*D$ for constants $C$ and $D$.
<<linear>>=
...
It is possible to implement your own drop out mechanisms. A \texttt{DropoutMechanism} object should be created which contains 5 elements. We show a toy example here and see \texttt{help(DropoutMechanism.object)} for further details.
...
my.example.dropout <- function(W){ #W is the user parameter
  ...
  #we return a named list with the following 5 elements
  retVal <- list(type="MNAR",  #technically this is MNAR dropout
  ...
  class(retVal) <- "DropoutMechanism" #see the class of the list
...
\subsection{Additional Impute Mechanisms}
