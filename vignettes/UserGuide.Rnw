...
We can output $\gamma$ and $\mu$ for the model fit (see Equation \eqref{nbpdf}) -- $\gamma$, a vector containing the value for first the control arm and then the active arm and $\mu$ a matrix, one row per subject\footnote{if there are covariates then $\mu$ is subject specific}, one column for if the subject were to have been in the control arm, the second for if the subject had been in the active arm:
<<ip>>=

gamma_and_mu <- my.fit$impute.parameters$gamma_mu_function()
...
steroidMCAR <- function(steroid.rate, non.steroid.rate,var=0){ 
  ...
  CreateNewDropoutMechanism(type="MCAR", 
...
gamma_mu <- dropout.fit$impute.parameters$gamma_mu_function()
head(gamma_mu$mu)
@
...
my.example.impute <- function(steroid.prob,non.steroid.prob){ 
  ...
  impute <- function(fit){
    ...
    for(id in 1:number.of.subjects){
      ...
      #Note could access the mu, gamma from the fit
      gamma_mu <-  my.fit$impute.parameters$gamma_mu_function()
...
Using the imputation mechanim, MI can be performed and models (with covariates) fitted to the data as before: 
<<>>=
