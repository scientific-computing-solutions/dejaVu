...
The \texttt{ConstantRateDrop} mechanism will give subjects exponentially distributed drop out times with subject specific rate $R$ where $\log(R) =$ \texttt{rate} $+ \mathcal{N}(0,$\texttt{var}$)$. If the simulated drop out time is $<T$ then the subject drops out and any events which occur after dropout are not observed.
...
\begin{itemize}
\item \texttt{observed.events ~ arm + offset(log(censored.time))} if \texttt{equal.dispersion} is \texttt{TRUE}
\item \texttt{observed.events ~ offset(log(censored.time))} if \texttt{equal.dispersion} is \texttt{FALSE} and separate models are fitted for each arm 
\end{itemize}

(Note: If covariates are required in the model, in will not be \textit{too} much work to allow users to specify the model formula)
...
We can output $\gamma$ and $p$ for the model fit (see Equation \eqref{nbpdf}) -- they are vectors containing the value first the control arm and then the active arm:
<<ip>>=
...
x <- summary(MI.answer,use.adjusted.pval=TRUE)
x$power
@
...
Explicitly, when using the \texttt{weighted\_j2r} function, with \texttt{trt.weight}$=\omega$, $p=(p_c,p_a)$ and $\gamma=(\gamma_c,\gamma_a)$ from the model fit for a subject with $O_i$ observed events and `imputation time' $t_i=T-$censor.time, the number of imputed events is a negative binomial given by
\begin{itemize}
